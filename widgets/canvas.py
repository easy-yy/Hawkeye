...
from widgets.feature_widget import FeatureWidget
import utils
...
class Canvas(QtWidgets.QWidget):
    ...
    # 单个多边形内部融合
    def mergeShapeInner(self):
        if self.firstPoint is None:
            self.firstPoint = self.prevMovePoint
        elif self.lastPoint is None:
            self.lastPoint = self.prevMovePoint
            shape = self.prevhShape
            if shape is None:
                return
            index1 = shape.nearestVertex(self.firstPoint, self.epsilon)
            index2 = shape.nearestVertex(self.lastPoint, self.epsilon)
            ...
            if index2 < index1:
                ...
                index1 = index2
                index2 = temp
            if index2 - index1 <= 1 or (index1 == 0 and index2 == (len(shape) - 1)):
                self.firstPoint = None
                self.lastPoint = None
                return
            if len(shape.points)-index2+index1-1 > index2-index1:
                for i in range(index1+1, index2):
                    shape.removePoint(index1+1)
                    self.hShape = shape
                    self.hVertex = None
                    self.hEdge = None
                    self.movingShape = True  # Save changes
                ...
                for i in range(index2+1, len(shape.points)):
                    shape.removePoint(index2+1)
                    self.hShape = shape
                    self.hVertex = None
                    self.hEdge = None
                    self.movingShape = True  # Save changzes
    ...
    def splitShape(self):
        if self.firstPoint is None:
            ...
            if index2 < index1:
                ...
                index1 = index2
                index2 = temp
            points = shape.points[index1: index2+1]
            if index2 - index1 <= 1 or (index1 == 0 and index2 == (len(shape) - 1)):
                self.firstPoint = None
                self.lastPoint = None
                return
