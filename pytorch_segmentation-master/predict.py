...
def main():
    config = json.load(open('saved/DeepLab/04-16_19-42/config.json'))
    ...
    device = torch.device('cuda:1' if len(availble_gpus) > 0 else 'cpu')

    # Load checkpoint
    # checkpoint = torch.load('saved/PSPNet/best_model.pth', map_location=device)
    checkpoint = torch.load('saved/DeepLab/04-16_19-42/best_model.pth', map_location=device)
    if isinstance(checkpoint, dict) and 'state_dict' in checkpoint.keys():
        checkpoint = checkpoint['state_dict']

    # If during training, we used data(old) parallel
    ...
    image_files = 'testimg/4444.png'
    output = 'outputs/predict1.png'
    output1 = 'outputs/predict.png'
    with torch.no_grad():
        ...
        # prediction = sliding_predict(model, input, num_classes, device)

        prediction = F.softmax(torch.from_numpy(prediction), dim=0).argmax(0).cpu().numpy()
        save_images(image, prediction, output, image_files, palette)
        cv2.imwrite(output1, prediction)
