...
def save_images(image, mask, output_path, image_file, palette, mode='maskOnly'):
...
def main():
    ...
    if 'module' in list(checkpoint.keys())[0] and not isinstance(model, torch.nn.DataParallel):
        # for gpu inference, use data(old) parallel
    ...
    # image_files = sorted(glob(os.path.join('testimg/1.png')))
    with torch.no_grad():
